...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollection<T> {
	...
	public void leftRotate(BinaryNode<T> subtree) throws NullPointerException {
		// left rotation
		//
		if (subtree == null || subtree.getLeft() == null) {
			throw new NullPointerException("Parent or child node is null");
		}

		BinaryNode<T> parent = subtree;
		BinaryNode<T> newParent = subtree.getRight();
		BinaryNode<T> grandparent = subtree.getUp();

		parent.setRight(newParent.getLeft());
		if (newParent.getLeft() != null) {
			newParent.getLeft().setUp(parent);
		}

		newParent.setLeft(parent);
		parent.setUp(newParent);

		newParent.setUp(grandparent);
		...
		if (grandparent != null) {
			...
			root = newParent;
			// assuming there is no grandparent and original parent was root
		}

	}

	public void rightRotate(BinaryNode<T> subtree) {
		// right rotation
		if (subtree == null || subtree.getLeft() == null) {
			throw new NullPointerException("Parent or child node is null");
		}
		BinaryNode<T> parent = subtree;
		BinaryNode<T> newParent = subtree.getLeft();
		BinaryNode<T> grandparent = subtree.getUp();

		parent.setLeft(newParent.getRight());
		if (newParent.getRight() != null) {
			newParent.getRight().setUp(parent);
		}
	...
	public boolean leftr_test() {

		this.insert((T) Integer.valueOf(5));
		this.insert((T) Integer.valueOf(3));
		this.insert((T) Integer.valueOf(7));
		this.insert((T) Integer.valueOf(6));
		this.insert((T) Integer.valueOf(9));

		this.leftRotate(root);

		String expectedLevelOrder = "[ 7, 5, 9, 3, 6 ]";
		String expectedInOrder = "[ 3, 5, 6, 7, 9 ]";
	...
	public boolean rightr_test() {
		...
		String expectedLevelOrder = "[ b, a, d, c, e ]";
		String expectedInOrder = "[ a, b, c, d, e ]";
	...
	public boolean nonRoot_test() {
		this.insert((T) Integer.valueOf(10));
		this.insert((T) Integer.valueOf(5));
		this.insert((T) Integer.valueOf(15));
		this.insert((T) Integer.valueOf(3));
		this.insert((T) Integer.valueOf(7));
		this.insert((T) Integer.valueOf(6));
		this.insert((T) Integer.valueOf(8));

		BinaryNode<T> node5 = root.getLeft();
		leftRotate(node5);
	...
	public static void main(String[] args) {
		...
		System.out.println(num_tree.leftr_test());

		BinarySearchTree<String> string_tree = new BinarySearchTree<>();
		System.out.println(string_tree.rightr_test());

		num_tree.clear();

		System.out.println(num_tree.nonRoot_test());
	}
}
